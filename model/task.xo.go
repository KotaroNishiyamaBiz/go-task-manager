...
// Task represents a row from 'go_todo.task'.
type Task struct {
	ID        uint      `json:"id"`         // id
...
func (t *Task) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO go_todo.task (` +
...
func (t *Task) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE go_todo.task SET ` +
		`created_at = ?, updated_at = ?, title = ?` +
		` WHERE id = ?`
...
func (t *Task) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM go_todo.task WHERE id = ?`
...
// TaskByID retrieves a row from 'go_todo.task' as a Task.
//
