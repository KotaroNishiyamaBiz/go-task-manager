// Package model contains the types for schema 'go_todo'.
package model
...
// GooseDbVersion represents a row from 'go_todo.goose_db_version'.
...
func (gdv *GooseDbVersion) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO go_todo.goose_db_version (` +
...
func (gdv *GooseDbVersion) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE go_todo.goose_db_version SET ` +
		`version_id = ?, is_applied = ?, tstamp = ?` +
		` WHERE id = ?`
...
func (gdv *GooseDbVersion) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM go_todo.goose_db_version WHERE id = ?`
...
// GooseDbVersionByID retrieves a row from 'go_todo.goose_db_version' as a GooseDbVersion.
//
